(function(d,p){
    ...
    var _doInit = function(){
        ...
        for(var i=_list.length-1,_script;i>=0;i--){
            ...
            _reg.test(_script.src)
                /*define1.如果是src中带有define.js，进行解析流程*/
    ...
    /*
     * 解析地址，将define.js?所在的url传入进行解析
     * @param  {String}
     * @return {Void}
     */
    var _doParseConfig = function(_uri){
        /*define2.根据相对路径，取出绝对路径*/
        ...
        /*define3.将define.js所在目录抽出*/
        __config.root.lib = _lib;
        /*define4.将其余路径配置参数发序列化为对象*/
    ...
    var _doLoadScript = function(_uri){
        /*df13创建script标签，并加入页面，进行模板文件的载入*/
        if (!_uri) return;
        var _state = __scache[_uri];
        if (_state!=null) return;
        // load file
            ...
            for(var i=0,l=_keys.length,_it,_list;i<l;i++){
                ...
                if (!!_list&&!!_list.length){
                    ...
                    for(var k=0,j=_list.length,_itt,_itm,_arr,_type;k<j;k++){
                        ...
                        // 1 - load function
                        // 2 - resource type
                        /*df11.根据解析模式不同，采用不同解析，
                         * 比如JSON资源采用json!前缀标识，
                         * Regular模板资源采用regular!前缀标识*/
                        _arr = _doParsePlugin(_itt);
                        _itm = _doFormatURI(_arr[0],_uri,_arr[2]);
                        /*df12.找到对应依赖模块的文件所在位置*/
                        _kmap[_itt] = _itm;
                        _list[k] = _itm;
                        // load resource
                        /*df12.执行加载函数_doLoadScript，加载模块对应的文件*/
                        _arr[1](_itm);
                    }
                    if (_it==='h'&&!!_xmap.f){
                        _xmap.f.kmap = _kmap;

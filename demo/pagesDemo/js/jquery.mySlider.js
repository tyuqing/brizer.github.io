...
;(function ($) {
            ...
            if(_inner>=_outer){ //只有在内层超过外层时移动
                ...
                switch(options.inEndEffect){
                    case 'cycle':
                        ...
                        $list1.animate(params,{easing:'linear', duration: duration});
                        params[_param] = _distance[_size]-_distance[_index];
                        $list2.animate(params,{easing:'linear', duration: duration, complete:function(){
                            if (_index >= _size) {
                                _index %= _size;
    ...
    $.fn.mySlider = function(parameter,callback) {
